...
const COLORS = [
	{ id: 'charcoal', label: 'CHARCOAL BLACK', bg: 'bg-[#101818]' },
			...
			<div className='relative z-10 mx-auto flex max-w-[900px] flex-col items-center text-center'>
				...
				<div className='mt-6 sm:mt-7 md:mt-8 flex flex-wrap items-center justify-center gap-4 sm:gap-6 md:gap-8'>
					{COLORS.map((color) => (
						<button
							key={color.id}
							className='flex items-center gap-3 sm:gap-4 rounded-[10px] sm:rounded-[12px] border border-white/25 px-3 sm:px-4 py-2 sm:py-3'>
							<span
								className={`h-7 w-7 sm:h-8 sm:w-8 md:h-9 md:w-9 rounded-full ${color.bg}`}
							/>
